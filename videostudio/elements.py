...
class Audio:
    def __init__(self, canvas, folder_path, start_asset, end_asset, fade_in_duration=0, fade_out_duration=0):
        self.folder_path = folder_path
        self.start_time = start_asset.start_time
        self.end_time = end_asset.end_time
    ...
    def _select_random_file(self):
        files = [f for f in os.listdir(self.folder_path) if os.path.isfile(os.path.join(self.folder_path, f)) and f.lower().endswith(('.mp3', '.wav', '.ogg', '.flac'))]
        if not files:
